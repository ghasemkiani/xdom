...
class Style extends Obj {
  ...
  toString() {
    return Object.entries(this.props)
      .filter(([k, v]) => !cutil.isNil(v))
      .map(([k, v]) => `${k}: ${cutil.isEmptyString(v) ? '""' : v};`)
      .join(" ");
  }
}
...
class X extends cutil.mixin(Obj, iwdom) {
  ...
  toText(node) {
    ...
    return x.kind(node) === "processingInstruction"
      ? ""
      : x.kind(node) === "document"
        ? x.toText(x.root(node))
        : x.kind(node) === "comment"
          ? ""
          : x.kind(node) === "element"
            ? x.toTextAll(x.nodes(node))
            : x.kind(node) === "text"
              ? node.data
              : x.kind(node) === "cdata"
                ? node.data
  ...
  toJs(node, level = 0, n = 2) {
    ...
    if (x.kind(node) === "element") {
      ...
      result += `${sp(level)}x.t(node, ${JSON.stringify(x.toText(node))});`;
    }
    return result;
  }
  ...
  env() {
    ...
    return [
      ...
      "escape",
      "unescape",
    ].reduce((env, k) => ((env[k] = x[k].bind(x)), env), {
      x,
      window,
      document,
    });
